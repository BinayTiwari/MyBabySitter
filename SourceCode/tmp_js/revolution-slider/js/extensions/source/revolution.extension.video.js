	...
	manageVideoLayer : function(_nc,opt,recalled,internrecalled) {				
		...
		switch (newvideotype) {
			case "html5":
				...
				if (videowebm!=undefined && _R.get_browser().toLowerCase()=="firefox") apptxt = apptxt + '<source src="'+videowebm+'" type="tmp_video/webm" />';
				if (videomp!=undefined) apptxt = apptxt + '<source src="'+videomp+'" type="tmp_video/mp4" />';
				if (videoogv!=undefined) apptxt = apptxt + '<source src="'+videoogv+'" type="tmp_video/ogg" />';
				apptxt = apptxt + '</'+tag+'>';
				var hfm ="";
